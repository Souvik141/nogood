      ...
      .exec((ahOh, ntt) => {
        if (ahOh) handleError(ahOh)
          ...
          formalEntity.findByIdAndUpdate(
            ntt._id,
            {
              firstName: rqst.body.firstName
                ? rqst.body.firstName
                : ntt.firstName,
              middleName: rqst.body.middleName
                ? rqst.body.middleName
                : ntt.middleName,
...
router.get("/get-image", (rqst, rspns, next) => {
  ...
  image.findById(mongoose.Types.ObjectId(rqst.query.id), (ahOh, img) => {
    if (ahOh) config.handleError(ahOh, rqst, rspns)
    if (!img) {
